...
void GetMacAddress()
{
    NCB Ncb;                                           //структура сетевого управляющего блока, 
                                                       //содержит данные о каманде, буфер для сообщений и т.д. 
                                                       //На ее основе происходит общение с интерфейсом NetBios.
...
BOOL WINAPI EnumerateFunc(LPNETRESOURCE lpnr)
{
    ...
    dwResult = WNetOpenEnum(RESOURCE_GLOBALNET,      // область применения.Перечислите все ресурсы в сети.
        ...
        0,                                           //Тип использования ресурса для перечисления.
                                                     //Если lpNetResource равен нулю, 
                                                     //то для перечисления будет начинаться с самого верха сети. 
                                                     //В другом случае этот параметр должен быть заполнен.
